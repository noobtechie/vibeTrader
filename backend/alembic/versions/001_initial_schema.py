...
def upgrade() -> None:
    ...
    # Candles (TimescaleDB hypertable — converted in env.py after migration)
    op.create_table(
        "candles",
    ...
    op.create_table(
        ...
        sa.Column("confidence_score", sa.Numeric(5, 2), nullable=False, default=0),
        sa.Column("automation_mode", sa.String(20), nullable=False),
        sa.Column("status", sa.String(20), nullable=False),
        sa.Column("candles_snapshot", postgresql.JSON(), nullable=True),
        sa.Column("pattern_meta", postgresql.JSON(), nullable=True),
